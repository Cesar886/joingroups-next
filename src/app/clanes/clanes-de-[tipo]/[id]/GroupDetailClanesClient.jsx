...
import {
  ...
  IconLanguage, IconSwords,
} from '@tabler/icons-react';

const API_URL = '';

function getDescriptionLanguages(description) {
  if (typeof description === 'string' && description.trim()) return 'No especificado';
  if (!description || typeof description !== 'object') return 'No especificado';

  const languages = [];
  if (description.es?.trim()) languages.push('Español');
  if (description.en?.trim()) languages.push('Inglés');

  return languages.length ? languages.join(' / ') : 'No especificado';
}

function getStoredRequiredTrophies(group) {
  const value = group?.requiredTrophies ?? group?.minimumTrophies ?? group?.minTrophies ?? group?.copasMinimas;
  return typeof value === 'number' ? value.toLocaleString() : null;
}

function getStoredCountry(group) {
  return group?.country || group?.pais || group?.país || group?.city || null;
}

function getActivityType(group) {
  if (Array.isArray(group?.categories)) return group.categories.filter(Boolean).join(', ');
  return group?.categories || group?.activityType || group?.tipoActividad || null;
}

function getWarParticipation(group) {
  if (typeof group?.participaGuerra === 'boolean') return group.participaGuerra ? 'Sí' : 'No';
  if (typeof group?.clanWar === 'boolean') return group.clanWar ? 'Sí' : 'No';
  if (typeof group?.war === 'boolean') return group.war ? 'Sí' : 'No';

  const activity = getActivityType(group);
  if (typeof activity === 'string' && /guerra/i.test(activity)) return 'Indicada en categoría';

  return 'No especificada';
}

export default function GroupDetailClanes({ initialGroup = null }) {
  ...
  useEffect(() => {
    const fetchGroup = async () => {
      try {
        ...
        if (snap.length === 0) { if (!initialGroup) setNotFound(true); return; }
        ...
        setGroup({ id: docSnap.id, ...publicData, slug: data.slug || slugify(data.name) });
      } catch (e) {
        console.error(e);
        if (!initialGroup) setNotFound(true);
      } finally {
        setLoading(false);
      }
    };
  ...
  const resolvedTipo = group?.tipo || group?.juego || tipo;
  const gameLandingHref = resolvedTipo === 'clash-royale' ? '/clanes/clanes-de-clash-royale' : '/clanes/clanes-de-clash-of-clans';
  const gameLabel = resolvedTipo === 'clash-royale' ? 'Clash Royale' : 'Clash of Clans';
  const memberList = Array.isArray(clan?.memberList) ? clan.memberList : [];
  const topTrophyMember = getTopBy(memberList, 'trophies');
  const storedRequiredTrophies = getStoredRequiredTrophies(group);
  const storedCountry = getStoredCountry(group);
  const storedActivity = getActivityType(group);
  const storedLanguages = getDescriptionLanguages(group?.description);
  const storedWarParticipation = getWarParticipation(group);
    ...
    <div className={classes.clanPageBg}>
      <div className={classes.pageWrapper}>
        <nav className={classes.breadcrumb} aria-label="Breadcrumb">
          ...
          <span aria-hidden="true">/</span>
          <Link href={gameLandingHref}>{gameLabel}</Link>
          <span aria-hidden="true">/</span>
          <span>{clanName}</span>
        </nav>
        ...
        <div className={classes.contentCard}>
          ...
          <div className={classes.sectionLabel}>Datos publicados</div>
          <div className={classes.statGrid}>
            <div className={classes.statTile}>
              <div className={classes.statTileLbl}><IconTrophy size={10} /> Copas mínimas</div>
              <div className={classes.statTileText}>{storedRequiredTrophies ?? 'No especificadas'}</div>
            </div>
            <div className={classes.statTile}>
              <div className={classes.statTileLbl}><IconMapPin size={10} /> País</div>
              <div className={classes.statTileText}>{storedCountry ?? 'No especificado'}</div>
            </div>
            <div className={classes.statTile}>
              <div className={classes.statTileLbl}><IconLanguage size={10} /> Idioma</div>
              <div className={classes.statTileText}>{storedLanguages}</div>
            </div>
            <div className={classes.statTile}>
              <div className={classes.statTileLbl}><IconSwords size={10} /> Guerra de clanes</div>
              <div className={classes.statTileText}>{storedWarParticipation}</div>
            </div>
            <div className={classes.statTile}>
              <div className={classes.statTileLbl}><IconStar size={10} /> Actividad</div>
              <div className={classes.statTileText}>{storedActivity ?? 'No especificada'}</div>
            </div>
          </div>
          <div className={classes.sep} />
          ...
          {clan && (
            <>
              <div className={classes.sectionLabel}>Estadísticas en vivo</div>
              <div className={classes.statGrid}>
                <div className={classes.statTile}>
          ...
          <div className={classes.internalLinks}>
            <Link href={gameLandingHref}>Ver más clanes de {gameLabel}</Link>
            <Link href="/clanes/publicar-clan">Publicar mi clan</Link>
          </div>
        </div>
      </div>
